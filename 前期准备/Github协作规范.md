...
2. **安装 Git**：Windows 下载 [Git Bash](https://git-scm.com/)，macOS 运行 `brew install git`。（可以用`git --version`检查是否下载）
3. **配置身份信息**（终端执行一次）：
   ```bash
...
| 规则                                        | 说明                                                      |
| :---------------------------------------- | :------------------------------------------------------ |
| 🔴 **绝对禁止直接** **`push`** **到** **`main`** | `main` 已开启保护，强推会报错且破坏版本记录                               |
| 🌿 **所有开发必须在独立分支**                        | 命名格式：`feature/姓名-功能` 或 `fix/姓名-问题`（如 `feature/Zichen_Zou-用户登录`） |
...
git add .                   # 添加所有改动到暂存区
git commit -m "feat: 完成用户登录接口与SQL查询 - Zichen Zou"  # 提交说明要清晰

# 4️⃣ 将你的分支推送到远程（此时 main 分支不会被修改！）
git push origin feature/Zichen_Zou-用户登录模块
```

> ✅ 执行完以上步骤后，**你的代码已安全备份到 GitHub**。下一步需在网页创建 PR 申请合并。
