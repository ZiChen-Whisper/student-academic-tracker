...
export default function ParentScore() {
  ...
  const classAvgBySubject = useMemo(() => {
    ...
    classStats.forEach((s) => {
      ...
      const rate = s.avg_score != null ? +(Number(s.avg_score) / (SUBJECT_FULL_SCORE[subj] || 100) * 100).toFixed(1) : null;
  ...
  const radarData = useMemo(() => {
    ...
    Object.keys(SUBJECT_MAP).forEach((subj) => {
      ...
      classRates[subj] = latest && latest.avg_score != null
        ? +(Number(latest.avg_score) / (SUBJECT_FULL_SCORE[subj] || 100) * 100).toFixed(1)
        : 0;
    });
