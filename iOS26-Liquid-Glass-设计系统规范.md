...
| 属性 | 值 |
|------|-----|
| 外观 | 使用 `<div role="combobox">` 而非 `<button>`（`<button>` 的原生渲染会阻止 `backdrop-filter` 生效，`appearance: none` 也无法解决） |
| 背景 | `rgba(255,255,255,0.55)` |
| 模糊 | `backdrop-filter: blur(12px) saturate(160%)` |
| 边框 | `0.5px solid rgba(11,101,101,0.1)` |
| 圆角 | `0.5rem` |
| 内边距 | `0.5625rem 0.875rem` |
| 字号 | `0.8125rem` |
...
### 5.15 悬停提示 `.liquid-tooltip`

替代浏览器原生 `title` 属性的自定义液态玻璃风格提示组件，通过 `createPortal` 渲染到 `document.body`。

**组件文件**：`frontend/src/components/LiquidTooltip.jsx`

**Hook**：`useLiquidTooltip()` — 返回 `{ tooltip, showTooltip, hideTooltip, moveTooltip }`

| 属性 | 值 |
|------|-----|
| 定位 | `position: fixed`，距鼠标右下方 `10px`，自动避免超出视口 |
| z-index | `10001` |
| 背景 | `rgba(255,255,255,0.72)` |
| 模糊 | `backdrop-filter: blur(20px) saturate(180%)` |
| 边框 | `0.5px solid rgba(11,101,101,0.12)` |
| 圆角 | `0.625rem` |
| 阴影 | `0 1px 3px rgba(11,101,101,0.06), 0 4px 12px rgba(11,101,101,0.04), 0 8px 24px rgba(11,101,101,0.03), inset 0 0.5px 0 rgba(255,255,255,0.6)` |
| 内边距 | `0.4375rem 0.75rem` |
| 字号 | `0.6875rem` |
| 文字色 | `var(--primary-dark)` #095050 |
| 字重 | 500 |
| 行高 | 1.5 |
| 最大宽度 | `260px` |
| 溢出 | `hidden`，`text-overflow: ellipsis`，`white-space: nowrap` |
| 交互 | `pointer-events: none` |
| 入场动画 | `0.15s ease`，`translateY(-3px) → 0` + `opacity 0 → 1` |

顶部高光线 `::before`：`left:10% right:10%`，`linear-gradient(90deg, transparent, rgba(255,255,255,0.8), transparent)`

底部反射光 `::after`：`left:15% right:15%`，`linear-gradient(90deg, transparent, rgba(11,101,101,0.06), transparent)`
...
**适用场景**：所有需要悬停提示的元素，统一替代浏览器原生 `title` 属性。
